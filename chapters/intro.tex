...
Fabric attains its maximum throughput at around 1500 tps when the number of transactions per block is 1000 and beyond. 
In the second experiment, we fix the block size at its optimal, i.e., 1000 transactions per block, and evaluate against the Smallbank workload~\cite{smallbank}. 
We first measure the peak performance without contention by uniformly creating bank accounts (loading records). 
As shown in the leftmost bar of Figure~\ref{chart:intro:basic:smallbank}, Fabric achieves similar throughout as in the previous YCSB experiment, around 1500 tps. 
Then we saturate the system with the transaction workload at a request rate of 1700 tps, which is slightly greater than the above-mentioned throughput.
The transaction workload simulates the monetary transfer between 100k bank accounts. 
And the accessed accounts(records) follow Zipfian distribution, whose skewness is proportional to its coefficient $\theta$. 
...
In particular, as illustrated by the blue bar, it only attains 580 tps when $\theta=1$, around 38.6\% of the peak. 
This baseline experiment again motivates for the performance speedup of permissioned blockchains. 
...
\begin{figure}[tp]
    ...
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
      \includegraphics[width=0.99\textwidth]{chart/intro/smallbank.pdf}
      \caption{Smallbank}
      \label{chart:intro:basic:smallbank}
    \end{subfigure}
    \caption{Primary evaluation results for Fabric. }
    \subcaption*{(a) Fabric attains around 1500 tps at 1000 transactions per block, reaching its peak under the contention-free workload. (b) The skewness of the workload impacts on the Fabric's behavior. }
