...
In this chapter, we design and implement a fine-grained, secure and efficient provenance system for blockchains,
...
The remainder of the chapter is organized as follows. Chapter~\ref{ch:provenance:background} provides necessary blockchain preliminaries for the understanding. Chapter~\ref{ch:provenance:capture} describes our design for capturing provenance, and the interface
exposed to smart contracts. Chapter~\ref{ch:provenance:storage} discusses how we store provenance, and
Chapter~\ref{ch:provenance:index} describes our new index. Chapter~\ref{ch:provenance:implementation} presents our
implementation. Chapter~\ref{ch:provenance:exp} reports the performance of {\fs} on provenance-related matters. 

\section{Preliminary}
\label{ch:provenance:background}
...
are unspent coins modeled as Unspent Transaction Outputs (UTXOs), as detailed in Chapter~\ref{ch:literature:datamodel}. More recent blockchains, namely Ethereum and Fabric, support general states that can be modified arbitrarily
...
\label{ch:provenance:storage:forward}
...
\subsection{Discussion} \label{ch:provenance:model_discuss}
...
affect the performance of a permissioned blockchain, as we demonstrate in Chapter~\ref{ch:provenance:exp}.  However,
...
\label{ch:provenance:implementation}
\begin{figure}
  \centering
...
and how to write user-defined provenance rules with it, are explained in Chapter~\ref{ch:provenance:capture}. The
...
% Appendix~\ref{ch:append:contracts:stub} and~\ref{ch:append:contracts:token} attaches the code of the provenance APIs introduced in \texttt{ChaincodeStubInterface}, and the full-fledged chaincode for the \textit{Token} contract in Figure~\ref{code:prov:enhanced_contract}

\section{Performance Evaluation}
\label{ch:provenance:exp}
\subsection{Baselines and Experimental Setup}
\label{ch:provenance:exp:setup}
...
Specifically for Smallbank, we implement the \texttt{ProvHelper} to capture the monetary flow as the state dependency.
% The code is attached in Appendix~\ref{ch:append:contracts:smallbank_prov}.
Secondly, we demonstrate the provenance-provided utility with the Smallbank workload. 
In the meantime, we dissect the Fabric bottleneck (the validation phase as identified in Chapter~\ref{ch:twin:exp:replication:model}) to understand the exact overhead. 
...
\label{ch:provenance:exp:peak}

Figure~\ref{chart:provenance:basic} presents the effective throughput for YCSB and Smallbank at their respective optimal setup, as previous identified in Chapter~\ref{ch:intro:basis}. 
...
For Smallbank, we implement the provenance helper method (which is detailed in Chapter~\ref{ch:provenance:capture}) to capture the monetary flow. 
...
We leave its answer to Chapter~\ref{ch:provenance:exp:util}, when we dissect the bottleneck. 
...
As we have demonstrated in Chapter~\ref{ch:twin:exp:replication:model}, the sequential validation rate decides on the system capacity. 
...
However, that storage in {\fsPrO} incurs a heavier role: as explained in Chapter~\ref{ch:provenance:exp:setup}, it tracks all data provenance. 
...
The provenance-provided utility is abundant. In the Smallbank contract, we additionally encode a provenance-dependent method \texttt{ConditionalRefund}.
% The code is attached in Appendix~\ref{ch:append:contracts:cond_refund}.
...
the separation of two phases is not only error-prone but also security-flawed, as explained in Chapter~\ref{ch:provenance:intro}.
...
The result further confirms the conclusion drawn in Chapter~\ref{ch:twin:exp:storage}: compared with the state storage, the ledger abstraction incurs the most storage overhead. 
