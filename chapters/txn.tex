...
serializability.
%
We have quantified this cost with respect to the Smallbank workload in Figure~\ref{chart:intro:basic:smallbank}.
%
Figure~\ref{chart:intro:basic:smallbank} report both the raw and effective peak throughputs under the contention-free workload(, as depicted in \textit{Load} bar) and the transaction workload with contention. 
%
...
%
In Figure~\ref{chart:intro:basic:smallbank}, a bar shows the raw throughput, while its blue part reports the effective throughput. 
%
As we can see, the transaction workload achieve similar raw throughput as in the contention-free workload. 
%
But with higher skewness, a substantial fraction of the in-ledger transactions are aborted for serializability. 
%
To be specific, this ratio grows to 57\% when $\theta=1$, severely reducing the system processing volume. 
...
%
\Cref{sec:txn:experiment} reports our experimental results.
...
%
We then dedicate~\Cref{sec:txn:impl:sharp} to discuss specific techniques for {\fs} , i.e., how to make use of ForkBase storage and make it compatible with the existing provenance support. 
...
In addition, the historical query powered by DASL in ~\Cref{prov:sec:index} can greatly facilitate the snapshot-based simulation in Algorithm~\ref{alg:txn:simulation}. 
...
In~\Cref{prov:sec:storage:forward}, we assume once creating $S_{A,5}$, the entries dependent on $S_{A,3}$ becomes permanent. So we dump all the information of these entries to $S_{A,5}$. 
