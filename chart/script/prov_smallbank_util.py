...
def main():
    ...
    fabricplus_delay = series[config.kFabricPlusLabel]
    series_offsets = [offsets[1]] * len(fabricplus_delay)
    xticks = config.sum_list(base_xticks, series_offsets) 
    ax.bar(xticks, fabricplus_delay, width=width, color=config.base_colors[config.kFabricPlusLabel], edgecolor='black',align='center', label=config.kFabricPlusLabel)

    fabricsharp_delay = series[config.kFabricSharpLabel]
    series_offsets = [offsets[2]] * len(fabricsharp_delay)
    xticks = config.sum_list(base_xticks, series_offsets) 
    ax.bar(xticks, fabricplus_delay, width=width, color=config.base_colors[config.kFabricSharpLabel], edgecolor='black',align='center', label=config.kFabricSharpLabel)
