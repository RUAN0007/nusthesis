...
def main():
    ...
    f.legend(fake_bars, ["Block Persistence", "State Persistence", "Txn Verification", "Other"],
             loc='upper left', ncol=1, bbox_to_anchor=(0.15, 0.91), fontsize=16) 

    fake_bars = []
    fake_bars.append(ax.bar(0, 0, color=config.kFabricColor))
    fake_bars.append(ax.bar(0, 0, color=config.kFabricPlusColor))
    fake_bars.append(ax.bar(0, 0, color=config.kFabricSharpColor))
    f.legend(fake_bars, [config.kFabricLabel, config.kFabricPlusLabel, config.kFabricSharpLabel],
             loc='upper left', ncol=1, bbox_to_anchor=(0.60, 0.91), fontsize=16) 
